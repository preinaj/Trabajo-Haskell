...
-- clustersDistanciaMinima2 :: Distancia -> [Cluster] -> ((Cluster, Cluster), Double)
-- clustersDistanciaMinima2 fdistancia vss = ( (vss!!nfmin, vss!!ncmin), min)
--     where   matrizProx = calculaMatrixProximidad2 fdistancia vss
--             ((nfmin, ncmin), min) = calculaMinimo matrizProx


-- calculaMinimo matrizProximidad = ((nfMin, ncMin), min)
--     where   listMatriz = toLists matrizProximidad
--             nfilas = nrows matrizProximidad
--             ncolumnas = ncols matrizProximidad
--             aplana = concat listMatriz
--             indListMatriz = zip [1..] aplana
--             (indMin, min) = head(sortBy sndTuple indListMatriz)
--             nfMin = (div indMin ncolumnas) + 1
--             ncMin = (div indMin nfilas) + 1
---------------------------------------------------------------------

d1 = clustersDistanciaMinima distEuclidea lc
d2 = clustersDistanciaMinima2 distEuclidea lc

clustersDistanciaMinima2 :: Distancia -> [Cluster] -> ((Cluster, Cluster), Double)
clustersDistanciaMinima2 fdistancia lclusters = head(sortBy sndTuple2 lclustersDist)
    where   matrizProx = calculaMatrixProximidad2 fdistancia lclusters
            lclustersDist = transformaMatriz matrizProx lclusters

sndTuple2 :: ((Cluster, Cluster), Double) -> ((Cluster, Cluster), Double) -> Ordering
sndTuple2 (x1,y1) (x2,y2)
    | y1 > y2 = GT 
    | otherwise = LT
...
-----------------------
lc :: [Cluster]
lc = [[array (1,2) [(1,2.0),(2,0.0)],array (1,2) [(1,2.0),(2,1.0)]],[array (1,2) [(1,10.0),(2,20.0)]],[array (1,2) [(1,15.0),(2,7.0)]]]
m = calculaMatrixProximidad2 distEuclidea lc

calculaMatrixProximidad2 :: Distancia -> [Cluster] -> Matrix Double
calculaMatrixProximidad2 fdistancia listClusters = calculaMatrixProximidadAux fdistancia lindClusters matrizacc
...
calculaMatrixProximidadAux fdistancia lindClusters matrizacc 
    ...
    where   headCluster = head lindClusters
            ...
            distClusterActual2Resto = calculaDistanciasAUnCluster2 fdistancia headCluster restoClusters
...
calculaDistanciasAUnCluster2 :: Distancia -> (Int, Cluster) -> [(Int, Cluster)] -> [( ( (Int, Cluster), (Int, Cluster) ), Double)]
calculaDistanciasAUnCluster2 fdistancia _ [] = []
calculaDistanciasAUnCluster2 fdistancia clusterH@(nch, ch) (clusterS@(nc, c):xss) = aplica : aplicaAgain
    where   aplica = ( (clusterH, clusterS) , distanciaEntreClusters fdistancia ch c)
            aplicaAgain = calculaDistanciasAUnCluster2 fdistancia clusterH xss
